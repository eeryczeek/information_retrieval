...
class Visualizator:
    ...
    def generate_combined_wordcloud(
        self, feature_names, article_name, all_article_vectorized_data
    ):
        """
        ...
        word_weights = {feature_names[j]: sums[j] for j in range(sums.shape[0])}
    ...
    def generate_wordcloud(self, feature_names, article_name, vectorized_data):
        ...
        """
        word_weights = {
            feature_names[j]: vectorized_data[0, j]
    ...
    def generate_radar_chart(self, labels, query_values, article_values, chart_name):
        ...
        ax.fill(angles, article_values, color="r", alpha=0.25, label="Stacked Articles")
        ax.set_yticklabels([])
    ...
    def visualize_results(
        ...
        scrap_vals,
        top_n=5,
        top_cooccurring_n=10,
    ):

        self.generate_combined_wordcloud(
            vectorizer.get_feature_names_out(), "query_combined_wordcloud", query_vals
        )

        self.generate_combined_wordcloud(
            vectorizer.get_feature_names_out(), "scrap_combined_wordcloud", scrap_vals
        )
        ...
        self.generate_scores_histogram(
            query_word_counts, "query_word_counts_histogram", log=True
        )

        self.generate_scores_histogram(
            scrap_word_counts, "scrap_word_counts_histogram", log=True
        )
        ...
        self.generate_scores_histogram(similarities, "similarity_scores_histogram")
        ...
        for article, similarity in top_n_articles:
            ...
            article_vectorized = (
                scrap_vals[scrap_keys.index(article)].toarray().flatten()
            )
